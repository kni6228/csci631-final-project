            ...
            if folder in directory_contents:
                ...
                for filename, bounding_box in json_bounding_boxes.items():
                    if len(filename.strip()) > 0:
                        imageSaved,outputPathSaved = write_image(original_data_path,output_dir,folder,filename,str(folders_map[folder]))
                        if imageSaved:
    ...
    with open("information.csv") as csv_file:
        ...
        for row in csv_reader:
            ...
            if folder in directory_contents:
                ...
                for filename, bounding_box in json_bounding_boxes.items():
                    if len(filename.strip()) > 0:
                        imageSaved,outputPathSaved = write_image2(original_data_path,output_dir,folder,filename,str(folders_map[folder]),bounding_box)
